  ...
  const handleWatchLater = async (WatchLaterId: string) => {
    try {
      ...
      setWatchLater((WatchLater) =>
        WatchLater.filter((item) => item._id !== WatchLaterId)
      );
    } catch (error) {
  ...
  if (!user) {
    return (
      <div className="text-center py-12">
        ...
        <p className="text-gray-600">Watch history isn't visible when signed out</p>
      </div>
    );
  }
        ...
        {watchLater.map((item) => (
          <div key={item._id} className="flex gap-4 group">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-gray-500 mt-1">watched {formatDistanceToNow(new Date(item.watchedon))} ago</p>
            </div>
            <DropdownMenu>
